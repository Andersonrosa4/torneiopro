...
const TournamentDetail = () => {
  ...
  const runSystemRulesGuard = useCallback((matchList: Match[], label: string): boolean => {
    ...
    if (violations.length > 0) {
      console.warn(`[SystemRulesGuard:${label}] ${violations.length} violação(ões) ignorada(s) para permitir lançamento livre:`);
      violations.forEach(v => console.warn(`  → [${v.rule}] ${v.message}`));
      return true; // permissivo: não bloqueia lançamentos
    }
    return true; // ok
  }, [tournament?.format]);
  ...
  const generateBracket = async (config: {
    ...
    if (config.useGroupStage) {
      ...
      if (isLuanaQuarters) {
        ...
        //   R1P2 (Jogo 26): 2D × 3A  → vencedor enfrenta 1C na Quartas Pos 3 (Jogo 32)
        //   R1P3 (Jogo 27): 2B × 3C  → vencedor enfrenta 1D na Quartas Pos 4 (Jogo 30)
        //   R1P4 (Jogo 28): 2C × 3B  → vencedor enfrenta 1A na Quartas Pos 1 (Jogo 29)
        const repechageMeta = [
          { pos: 1, leftGroup: 0,             rightGroup: numGroups - 1, quarterPos: 2 },             // 2A×3D → Q2 (1B)
          { pos: 2, leftGroup: numGroups - 1, rightGroup: 0,             quarterPos: numGroups - 1 }, // 2D×3A → Q3 (1C)
          { pos: 3, leftGroup: 1,             rightGroup: numGroups - 2, quarterPos: numGroups },     // 2B×3C → Q4 (1D)
          { pos: 4, leftGroup: numGroups - 2, rightGroup: 1,             quarterPos: 1 },             // 2C×3B → Q1 (1A)
        ];
